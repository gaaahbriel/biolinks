<x-layout.app>
    <x-container>
        <div class="absolute top-10 left-10 flex flex-col gap-4">
            <x-button ghost :href="route('profile')">Edit Profile</x-button>
            ...
            <ul class="space-y-2">
                ...
                <li class="flex item-center gap-2">
                    ...
                    <x-button disabled ghost>
                        <x-icons.arrow-down class="w-6 h-6 opacity-0" />
                    </x-button>
                    @endunless
                    ...
                    @else
                    <x-button disabled ghost>
                        <x-icons.arrow-up class="w-6 h-6 opacity-0" />
                    </x-button>
                    @endunless


                    <x-button :route="route('links.edit', $link)" block outline info>
                        {{ $link->name }}
                    </x-button>

                    <x-form :route="route('links.destroy', $link)" delete onsubmit="return confirm('Tem certeza que deseja deletar este link?');">
                        <x-button ghost>
                            <x-icons.trash class=" w-6 h-6"></x-icon>
                        </x-button>
                    </x-form>
                </li>
                @endforeach
